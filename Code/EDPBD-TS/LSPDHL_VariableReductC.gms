...
*     );
parameter HubCost2(hubs);
HubCost2(hubs) = %HBCST%*10*uniformint(200, 1000);
HubCost(hubs, periods) = HubCost2(hubs);


parameter       rhubC(hubs)    Rank values,
...
$libinclude rank HubCost2 hubs rhubC pct
display HubCost2, rhubC, pct;
parameter removepct;
removepct = 0.7 ;
parameter removeranking;
removeranking = (1 - removepct)*card(hubs);  
...
file LSPDHLVarReduction /LSPDHLVarReduction.dat/;
put LSPDHLVarReduction;
LSPDHLVarReduction.ap = 1;
...
equations
master_obj2
cut2(iter, products, periods) 'Benders cut for optimal subproblem'
HubOpenMstr2
;
binary variable Zaggr(hubs);

master_obj2..  MasterCost =e= sum((products, periods), zetavar(products, periods)) + sum((hubs, periods), HubCost(hubs, periods)*Zaggr(hubs))
                                 + sum((plants, products), total_product_cost(plants, products)*U(plants, products)) ;

cut2(cutset, products, periods)..
              zetavar(products, periods) =g= cutconst(cutset, products, periods) - sum((hubs), cutcoeff1(cutset, hubs, products, periods)*Zaggr(hubs))
                          - sum((plants), cutcoeff2(cutset, plants, products, periods)*U(plants, products))  ;

HubOpenMstr2..
         sum(hubs, Zaggr(hubs)) =g= 1 ;

model master2 /master_obj2, cut2, plantloc, HubOpenMstr2/;
* reduce output to listing file:
master2.solprint=2;
* speed up by keeping GAMS in memory:
master2.solvelink=2;
...
parameter ETwarmupphase, probthreshold;
ETwarmupphase = 0;
probthreshold = 0.01;
parameter warmupLB;
warmupLB = -inf;
set removedhubs2(hubs, periods);
removedhubs2(hubs, periods) = no;
...
loop(products,
         ...
         loop(hubs, loop(periods,
                    if(zsol(hubs, periods) eq 1,
                           interiorzsol(hubs, periods) = 0.99 ;
                           else
                           interiorzsol(hubs, periods) = 0.01 ;
                       );
              ););

         loop(plants,
                    if(usol(plants, products) eq 1,
                           interiorusol(plants) = 0.99 ;
                           else
                           interiorusol(plants) = 0.01 ;
                       );
              );
...
if(ETwarmupphase eq 0 and ord(iter) le 8,
      removedhubs2(hubs, periods)$(rhubC(hubs) gt removeranking) = yes;     
      Z.fx(hubs, periods)$removedhubs2(hubs, periods) = 0;
      solve master minimizing MasterCost using mip;
      totaltime = totaltime + master.etSolve;

*      Z.fx(hubs, periods)$removedhubs2(hubs, periods) = 0;
*      zsol(hubs, periods) = Zaggr.l(hubs) ;
*      usol(plants, products) = U.l(plants, products) ;
        
      warmupLB = master.ObjEst;
      if(UB - warmupLB le 0.01,
             ETwarmupphase = 1;
          );
      else
             solve master minimizing MasterCost using mip;
             totaltime = totaltime + master.etSolve;
             if(master.ObjEst ge LB,
             LB = master.ObjEst;
             );
...
put  %ID%, %PLTSET%, %PRTSET%, %PRSET%, %HBSET%, totalopenhubs, totalprodcost,  " ", totalhubcost,  " ", totaltransportcost, " ",  iteration, "  ", LB, " ", UB, " ", heurBestObj, " ", Heuristictime, " ", totaltime, " ", warmupLB ///
);
